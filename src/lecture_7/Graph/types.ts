...
export interface IGraph<T> {
	...
	/**
	 * Возвращает итерируемый объект для перебора вершин графа по стратегии DFS, но
	 * вершины итерируются в обратном порядке, то есть в порядке их полной обработки
	 */
	getDFSReversedIterator(): Iterable<T>;
...
export enum Errors {
	...
	UNABLE_FIND_SHORT_PATH = 'Unable to find short path. Graph must be directed',
	PATH_NOT_FOUND = 'Path is noy found. Check start end end nodes',
}
